...
MongoDB.prototype.getDefaultIdType = function() {
  // Changed from ObjectID to String to provide a string interface of ObjectID to our users.
  // We take care of String-to-ObjectID and ObjectID-to-String coercion.
  return String;
};
...
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  // modelCtor is not applicable for nested objects
  // This is required because of the this.fromDatabase() recursion
  if (modelCtor) {
    visitAllProperties(data, modelCtor, coercePropertyForUser);
  }
  return data;
};
...
MongoDB.prototype.toDatabase = function(modelName, data) {
  ...
  if (this.settings.enableGeoIndexing !== true) {
    visitAllProperties(data, modelCtor, coercePropertyForDatabase);
...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  const propDef = self.getPropertyDefinition(modelName, idName);

  if (hasDataType('objectid', propDef) && !propDef.generated && idValue) {
    if (!ObjectIdValueRegex.test(idValue)) {
      const err = new Error('Invalid ObjectId string.');
      return callback(err);
    }
  } else if (propDef.generated && idValue) {
    const err = new Error('Cannot specify value for autogenerated id.');
    return callback(err);
  } else if (propDef.generated === undefined && idValue === undefined) {
    const err = new Error('Value is required for non-autogenerated id.');
    return callback(err);
  }
  ...
  if (idValue === null) {
    ...
    const oId = coercePropertyToObjectId(propDef, idValue);
  ...
  this.execute(modelName, 'insertOne', data, buildOptions({safe: true},
    ...
    // id value comes as ObjectID from database
    const oId = result.ops[0]._id;
    let sId;
    try {
      const propDef = self.getPropertyDefinition(modelName, idName);
      // Coerce idValue to string for user consumption, as a result any code using
      // Model.id will have a string value, therefore coerce before querying the database
      sId = coercePropertyToString(propDef, oId);
    } catch (err) {
      return callback(err);
    }

    // Wrap it to process.nextTick as delete data._id seems to be interfering
    // with mongo insert
    process.nextTick(function() {
      // Restore the data object
      delete data._id;
      data[idName] = sId;
      callback(err, err ? null : sId);
    });
  });
};
...
MongoDB.prototype.exists = function(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  const propDef = self.getPropertyDefinition(modelName, idName);
  id = coercePropertyToObjectId(propDef, id);
...
MongoDB.prototype.find = function find(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  const propDef = self.getPropertyDefinition(modelName, idName);
  const oid = coercePropertyToObjectId(propDef, id);
...
MongoDB.prototype.buildWhere = function(modelName, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (hasDataType('decimal128', propDef)) {
      ...
      cond = coercePropertyToObjectId(propDef, cond);
    // For autogenerated ids
    } else if (propDef) {
      if (cond.inq && Array.isArray(cond.inq)) {
        cond.inq = cond.inq.map(item => coercePropertyToObjectId(propDef, item));
      } else {
        cond = coercePropertyToObjectId(propDef, cond);
      }
    }
 ...
 */
function coercePropertyForDatabase(modelCtor, propValue, propDef, setValue) {
  if (propDef) {
    let coercedValue;
    if (hasDataType('decimal128', propDef)) {
      ...
      coercedValue = coercePropertyToObjectId(propDef, propValue);
      return setValue(coercedValue);
    }
  } else {
...
function coercePropertyForUser(modelCtor, propValue, propDef, setValue) {
  if (propDef) {
    const uncoercedValue = coercePropertyToString(propDef, propValue);
    return setValue(uncoercedValue);
  } else {
    return setValue(propValue);
  }
}

// Coerce a property to ObjectID if it is defined as one
function coercePropertyToObjectId(propDef, propValue) {
...
// Coerce a property (ObjectID) to String for user consumption
function coercePropertyToString(propDef, propValue) {
  if (propValue == null) return propValue;
